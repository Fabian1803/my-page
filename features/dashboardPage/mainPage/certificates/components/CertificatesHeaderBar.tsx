...
interface CertificatesHeaderBarProps {
    isLoading: boolean;
    filterTab: 'all' | 'destacados';
    onToggleFilterTab: () => void;
    onRefresh: () => void;
}

export default function CertificatesHeaderBar({
    isLoading,
    filterTab,
    onToggleFilterTab,
    onRefresh
}: CertificatesHeaderBarProps) {
    return (
        <div className="flex justify-between border-b border-[#dbdce0] px-4 sm:px-6 pt-3 pb-2 bg-white rounded-t-2xl">
            <div className="flex gap-2 sm:gap-3 justify-between w-full sm:w-auto items-center">
                <h1 className="text-base sm:text-lg font-medium text-gray-800 truncate">Mis Certificados</h1>
                <div className="flex items-center gap-2">
                    ...
                    <button
                        ...
                        className={`hidden sm:flex items-center gap-1.5 hover:bg-gray-200 transition px-2 py-1 text-[#0c68e0] hover:text-blue-700 rounded-sm cursor-pointer text-xs sm:text-sm font-medium ${
                            filterTab === 'destacados' ? 'bg-blue-50 font-semibold' : ''
                        }`}
                    >
                        <CiBookmarkRemove size={16} />
                        <span>{filterTab === 'destacados' ? 'Ver todos' : 'Ver destacados'}</span>
                    </button>
                </div>
            </div>
            ...
            <div className="hidden sm:flex gap-4 items-center">
                ...
                <div
                    ...
                    title="Actualizar lista de certificados"
                >
