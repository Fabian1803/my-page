...
export class CreateMetadataUseCase {
    ...
    async execute(request: Request) {
        ...
        if (imagenFile && imagenFile.size > 0) {
            if (metadatosActuales.url_imagen) {
                try {
                    await this.mediaStorage.deleteFile(metadatosActuales.url_imagen);
                } catch {}
            }
        ...
        if (pdfFile && pdfFile.size > 0) {
            if (metadatosActuales.url_cv_pdf) {
                try {
                    await this.mediaStorage.deleteFile(metadatosActuales.url_cv_pdf);
                } catch {}
            }
        ...
        await this.repository.save(metadataEntity);
        return metadatosFinales;
    }
}